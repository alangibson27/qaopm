...
class TestFetchBytes(TestHelper):
    def test_fetch_bytes_fetches_contiguous_bytes_from_pc_less_than_0xfffb(self):
        # given
        self.memory[0x1000] = 1
        self.memory[0x1001] = 2
        self.memory[0x1002] = 3
        self.memory[0x1003] = 4
        self.memory[0x1004] = 5
        self.memory[0x1005] = 6

        self.given_program_counter_is(0x1000)

        # when
        bytes = self.processor.fetch_bytes()

        # then
        assert_equals(bytes, [1, 2, 3, 4, 5, 6])

    def test_fetch_bytes_wraps_around_bytes_when_pc_is_0xfffb(self):
        # given
        self.memory[0xfffb] = 1
        self.memory[0xfffc] = 2
        self.memory[0xfffd] = 3
        self.memory[0xfffe] = 4
        self.memory[0xffff] = 5
        self.memory[0x0000] = 6

        self.given_program_counter_is(0xfffb)
    ...
    def test_fetch_bytes_fetches_wraps_around_bytes_when_pc_is_0xfffe(self):
        # given
        self.memory[0xfffe] = 1
        self.memory[0xffff] = 2
        self.memory[0x0000] = 3
        self.memory[0x0001] = 4
        self.memory[0x0002] = 5
        self.memory[0x0003] = 6

        self.given_program_counter_is(0xfffe)
